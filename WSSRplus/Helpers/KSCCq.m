...
while iter <= max_iter
    ...
    for k = 1:K
        ...
        if size(X(asmt==k,:), 1) == 1 % if there is only one point in the cluster
            re(:,k) = sum((X-repmat(X(asmt==k,:),N,1)).^2, 2);
            mu(k,:) = X(asmt==k,:);
            vec_all{k} = [];
            val_all{k} = 0;
        elseif size(X(asmt==k,:), 1) == 0 % if no point is assigned to this cluster 
            %re(:,k) = sum(X.^2, 2);
            ind = randsample(1:N, 1);
            mu(k,:) = X(ind,:);
            val_all{k} = [1; zeros(P-1, 1)];
            vec_all{k} = repmat(X(ind,:)', 1, P);
            re(:,k) = sum((X-repmat(X(ind,:),N,1)).^2, 2);
        else
