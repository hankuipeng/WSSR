% This function solves the WSSR problem via accelerated proximal /
% projected subgradient descent.

function [W obj_stars] = WSSR_APSD(X, k, rho, normalize, denom, MaxIter)

% MaxIter: the maximum number of iterations to run projected subgradient
% descent for; 
...
for i = 1:N
    ...
    for iter = 1:MaxIter
        ...
        % step 2.1 the accelerated step
        if iter > 1
            omega_cur = iter/(iter+3);
            beta_cur = beta_cur + omega_cur*(beta_cur-betas(:,iter-1));
        end
        
        % step 2.2: calculate the subgradient
